...
{
    uint8_t Step; // 通信收发状态
    uint8_t DelayCnt;      // 延时计数
    uint8_t TxIndex;  //发送缓存数据索引
    uint8_t RxIndex;  //接收缓存数据索引
    uint8_t TxLength; // 发送字节总数
    uint8_t RxLength; // 接收字节总数
    uint8_t RxPayloadLength; // 接收字节总数
    uint8_t SetNetSteps; //配网步骤
    uint8_t CloseRequest; // wifi配网关闭的回复
    uint32_t DisnetCnt;
    uint16_t Cnt;
    //bool TxDoneFlag; //发送完成标志
    //bool RxDoneFlag; //接收完成标志
    //u8 ErrFlag;         // 通讯故障标志 30秒无接收则故障停机
    //u8 ErrCnt;          // 故障计数
} WIFI_COMM_DEF;

extern  WIFI_COMM_DEF WIFI_CommVars;
